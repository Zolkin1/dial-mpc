...
@dataclass
class UnitreeG1StairsEnvConfig(BaseEnvConfig):
    kp: Union[float, jax.Array] = field(default_factory=lambda: jnp.array(
        [
            ...
            200.0,
            30.0,
            30.0,  # left knee, ankles
            200.0,
            200.0,
            200.0,  # right hips
            200.0,
            30.0,
            30.0,  # right knee, ankles
            # 200.0,
            # 200.0,
            # 200.0,  # torso
            60.0,
            ...
            # 60.0,
            # 60.0,
            # 60.0,  # left shoulder, elbow, wrists
            60.0,
            60.0,
            60.0,
            60.0,
            # 60.0,
            # 60.0,
            # 60.0,  # right shoulder, elbow, wrists
        ]
    ))
    kd: Union[float, jax.Array] = field(default_factory=lambda: jnp.array(
        [
            ...
            # 1.5,
            # 1.5,
            # 1.5,  # left shoulder, elbow, wrists
            1.5,
            1.5,
            1.5,
            1.5,
            # 1.5,
            # 1.5,
            # 1.5,  # right shoulder, elbow, wrists
        ]
    ))
...
class UnitreeG1StairsEnv(BaseEnv):
    def __init__(self, config: UnitreeG1StairsEnvConfig):
    ...
    def make_system(self, config: UnitreeG1StairsEnvConfig) -> System:
        model_path = get_model_path("unitree_g1", "stairs_scene.xml")
    ...
    def reset(self, rng: jax.Array) -> State:
        ...
        state_info = {
            "rng": rng,
            "pos_tar": jnp.array([0.0, 0.0, 0.77]),
            "vel_tar": jnp.zeros(3),
...
brax_envs.register_environment("unitree_g1_stairs", UnitreeG1StairsEnv)
dial_envs.register_config("unitree_g1_stairs", UnitreeG1StairsEnvConfig)